...
const AssignmentReport = () => {
  ...
  const reportListApi = async () => {
    ...
    const url =
      startDate != "Select" && endDate != "Select"
        ? endurl.PARENT_REPORT_ASSIGNMENT_LIST +
          `?start_date=${startDate}&end_date=${endDate}&student_id=${studentId}`
        : endurl.PARENT_REPORT_ASSIGNMENT_LIST + `?student_id=${studentId}`;
    ...
    if (await isPermitted()) {
      let options = {
        ...
        `
          ...
          .join("")}</table></div>`,
        //File Name
        fileName: "assignment_report_parent",
        //File directory
        directory: "docs",
      };
  ...
  return (
    <SafeAreaView style={styles.mainView}>
      <BackHeaderForParent title={CONSTANT.REPORT_ASSIGNMENT} dropdown={true} />
