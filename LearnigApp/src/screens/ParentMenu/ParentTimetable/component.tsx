...
export const CurrentMonth = ({ setFlatLlistData }: any) => {
  ...
  const { data , school_id} = loginData;

  const asssigned_class = useSelector(
    (state: any) => state.dropdown.data.assign_class
  );
  ...
  const getClassDataDropDown = () => {
    getAPIRequest(endurl.CLASSLIST + `?limit=1000&school_id=${school_id}`)
      .then((res: any) => {
  ...
  // http://150.129.243.95:8000/api/v1/roster-service/time_tables?start_date=2023-06-15&end_date=2023-06-21&class_id=8
