...
const StudentAssignmentReport = () => {
    ...
    if (await isPermitted()) {
      let options = {
        ...
        `
          ...
          .join("")}</table></div>`,
        //File Name
        fileName: "assignment_report_student",
        //File directory
        directory: "docs",
      };
  ...
  const reportListApi = async () => {
    ...
    const url =
      startDate != "Select" && endDate != "Select"
        ? endurl.STUDENT_REPORT_ASSIGNMENT_LIST +
          `?start_date=${startDate}&submission_date=${endDate}&student_id=${student_id}`
        : endurl.STUDENT_REPORT_ASSIGNMENT_LIST + `?student_id=${student_id}`;
  ...
  return (
    <SafeAreaView style={styles.mainView}>
      <BackHeader title={CONSTANT.REPORT_ASSIGNMENT} />
      ...
      <View style={localStyles.studentNameView}>
        <Text style={localStyles.studentName}>{student_name}</Text>
      </View>
