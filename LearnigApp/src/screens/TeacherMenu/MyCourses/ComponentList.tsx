...
const ComponentList = (props: any) => {
  ...
  const { title, grade, createAssignmentData  } = props?.route?.params
      ...
      Promise.all(promises)
        .then(responses => {
          ...
          const mappedResponses = responses.map((res, index) => {
            ...
            createAssignmentData: { ...createAssignmentData, componentSelectData:  diffPoints[index].component},
          })});
  ...
  const onSelect = (title: string, data: any, createAssignmentData: any) => {
    navigation.navigate(NAVIGATION_CONSTANT.Courses_Table_Of_Contents, {
      course_id: data?.value,
      title: headerTitle + ' ' + data?.label,
      createAssignmentData: {...createAssignmentData, volumeSelectData: data?.label }
    })
  }
      ...
      <View style={{
        ...
        <View style={{ flexDirection: 'row', }}>
          ...
          <View style={styles.dropDown}>
            ...
            title={"Select Volume"}
            data={item.response}
            onSelect={(data) => onSelect(item?.name, data, item?.createAssignmentData)}
          />  
          </View>
        </View>
      </View>
