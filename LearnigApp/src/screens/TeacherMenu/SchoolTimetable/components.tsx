...
export const CurrentMonth = ({ setFlatLlistData }: any) => {
  ...
  useEffect(()=> {
    console.log("Slected dates - ",selectedDates);
    
    callTimeTableAPI(selectedDates);
  },[typeData]);
  ...
  const getClassDataDropDown = () => {
    ...
    getAPIRequest(
      ...
      .then((res: any) => {
        setClassList(res?.data?.response?.results);
      })
      .catch((error) => {
        console.log("Getclass dropdown API failed- ",error.response.data);
  ...
  const callTimeTableAPI = (dates: any) => {
    ...
    // getAPIRequest(
    //   endurl.TIMETABLE +
    //     `?start_date=${startDate}&end_date=${endDate}&class_id=${
    //       loginData?.role?.name == "Teacher" ? "1805" : typeData?.id
    //     }&branch_id=3702`
    // )
    console.log("API call - ",endurl.TIMETABLE +
    `?start_date=${startDate}&end_date=${endDate}&class_id=${typeData?.id}&branch_id=${loginData?.branch_id ? loginData?.branch_id  : ''}`);
  ...
  const handleMonthChange = (month: element) => {
    ...
    console.log("handleMonthChange selectedMonthNumber - ",selectedMonthNumber + "\t selectedWeek- "+ JSON.stringify(selectedWeek));
    
    const d = new Date();
    let year = d.getFullYear();
    getWeekDates(year, selectedMonthNumber -1, parseInt(selectedWeek.value)); // month is 0-indexed, so 4 = May
